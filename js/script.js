...
(function () {
  ...
  const MAX_HP = 100;
  const ATTACKS = {
    facil: { name: 'Fácil', dmg: 10 },
    medio: { name: 'Médio', dmg: 20 },
    dificil: { name: 'Difícil', dmg: 30 }
  };
  ...
  function init() {
    ...
    const btnStartText = btnStart ? btnStart.textContent : 'Iniciar';
    if (btnStart) { btnStart.disabled = true; btnStart.textContent = 'Carregando...'; }

    // Load questions JSON (with fallback if fetch blocked on file://)
    function loadQuestions() {
      return fetch('data/perguntas.json')
        .then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
        .then(data => { questions = data; console.log('Perguntas carregadas', Object.keys(questions)); })
        .catch(err => {
          ...
          questions = {
            facil: [{ pergunta: "f(x)=x², f'(x)=?", alternativas: ['2x', 'x²', 'x', '1'], correta: '2x' }],
            medio: [{ pergunta: "f(x)=x² sin x, f'(x)=?", alternativas: ['2x sin x + x² cos x', '2x cos x', 'x² cos x', '2 sin x'], correta: '2x sin x + x² cos x' }],
            dificil: [{ pergunta: "∫ x eˣ dx", alternativas: ['eˣ(x-1) + C', 'eˣ x + C', 'x eˣ + C', 'eˣ + C'], correta: 'eˣ(x-1) + C' }]
          };
        })
        .finally(() => {
          if (btnStart) { btnStart.disabled = false; btnStart.textContent = btnStartText; }
        });
    }
  ...
  function pickQuestion(difficulty) {
    // difficulty is 'facil', 'medio', or 'dificil'
  ...
  function closeQuestion() {
    ...
    clearTimer();
    // ensure we are no longer awaiting an answer (e.g., when skipped)
    awaitingAnswer = false;
    // reset alternative buttons state so UI is consistent
    const altBtns = $$('.alt-btn');
    altBtns.forEach(b => {
      b.disabled = false;
  ...
  function showAttackEffect(attackType, position) {
    ...
    } else if (attackType === 'medio') {
      effect.className = 'effect-energia';
    } else if (attackType === 'dificil') {
  ...
  function playAttackSound(atk) {
    // different tones per difficulty level
    if (atk === 'facil') playBeep(860, 0.12, 'square', 0.06);
    else if (atk === 'medio') { playBeep(620, 0.18, 'sawtooth', 0.08); playBeep(900, 0.09, 'sine', 0.04); }
    else if (atk === 'dificil') { playBeep(420, 0.28, 'sine', 0.12); playBeep(240, 0.12, 'sine', 0.06); }
  }
